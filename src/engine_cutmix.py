...
## https://github.com/clovaai/CutMix-PyTorch/blob/2d8eb68faff7fe4962776ad51d175c3b01a25734/train.py#L231
...
def train(train_loader, model, loss_func, metric_func, device, optimizer, use_csv, use_amp):
            ...
            if use_amp == True:
                with torch.cuda.amp.autocast():
                    if np.random.random() > 0.5:
                        ...
                        image, label = train_x.clone(), train_y.clone()
                        ...
                        for idx in range(image.size()[0]):
                            if bbx2 - bbx1 > 0 and bby2 - bby1 > 0 :
                                image[idx,:,bbx1:bbx2, bby1:bby2] = torchvision.transforms.Resize(size=(bbx2-bbx1, bby2-bby1))(train_x[rand_index[idx]])
                        ...
                        if use_csv == True:
                            csv = train_csv.clone()
                            max_len = csv.size()[1]
                            if int(max_len*(1-lam)) > 0:
                                csv[rand_index, :int(max_len*(1-lam))] = train_csv[rand_index, -int(max_len*(1-lam)):]

                            output = model(image, csv)
                        else :
                            output = model(image)

                        loss = loss_func(output, label) * lam + loss_func(output, shuffled_label) * (1. - lam)
...
class ModelTrainer:
    def __init__(self, model, train_loader, valid_loader, loss_func, metric_func, optimizer, device, save_dir, 
                       mode='max', scheduler=None, num_epochs=25, num_snapshops=None, parallel=False, use_csv=True, use_cutmix=True, use_amp=True, use_wandb=True):
